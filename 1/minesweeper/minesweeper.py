...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for x in range(max(0, i - 1), min(self.height, i + 2)):
            for y in range(max(0, j - 1), min(self.width, j + 2)):
                if (x, y) != (i, j) and (x, y) not in self.moves_made and (x, y) not in self.safes:
                    neighbors.add((x, y))
        ...
        for sentence in self.knowledge:
            for other_sentence in self.knowledge:
                if sentence != other_sentence and sentence.cells:
    ...
    def update_knowledge(self):
        """
        Update knowledge base by marking additional cells as safe or as mines if possible.
        """
        while True:
            new_inferences = []
            for sentence in self.knowledge:
                ...
                if known_mines:
                    new_inferences.extend([(cell, 'mine') for cell in known_mines])
                if known_safes:
                    new_inferences.extend([(cell, 'safe') for cell in known_safes])

            if not new_inferences:
                break

            for cell, cell_type in new_inferences:
                if cell_type == 'mine':
                    self.mark_mine(cell)
                elif cell_type == 'safe':
                    self.mark_safe(cell)
